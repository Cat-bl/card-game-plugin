...
const PLUGIN_ROOT = path.join(process.cwd(), 'plugins/card-game-plugin')
...
function ensureUserConfig() {
  ...
  for (const name of fs.readdirSync(DEFAULT_DIR)) {
    ...
    if (!fs.existsSync(userFile)) {
      ...
      logger?.mark(`[卡牌游戏] 创建默认配置 config/${name}`)
    }
  }
}
...
function readYaml(file) {
  try {
    ...
    logger?.error(`[卡牌游戏] 解析配置失败 ${file}:`, err?.message || err)
    return {}
  }
}
...
class Config {
  ...
  watch(name, file) {
    ...
    watcher.on('change', () => {
      logger?.mark(`[卡牌游戏] 配置文件变更，热更新 config/${name}.yaml`)
